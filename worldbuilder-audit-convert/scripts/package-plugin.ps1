...
<#
.SYNOPSIS
  Runs Epic's RunUAT BuildPlugin for WorldBuilder Audit & Convert (Windows editor plugin).

.PARAMETER EngineRoot
  Root of the Unreal installation (folder that contains Engine\Build\BatchFiles\RunUAT.bat).

.PARAMETER UeVersionLabel
  Short label for the output folder name (e.g. "5.4", "5.5").
...
$PluginFile = Join-Path $ProductRoot "WorldBuilderAuditConvert\WorldBuilderAuditConvert.uplugin"
...
$PackageName = "WorldBuilderAuditConvert-UE$UeVersionLabel-Win64"
...
Write-Host "Done. Smoke-test on a clean project before Fab upload (see docs/PACKAGING_FOR_FAB.md)."
